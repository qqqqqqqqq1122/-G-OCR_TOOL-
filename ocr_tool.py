...
def recognize_shortcut():
    ...
    # 先把剪贴板里的旧图片顶掉，换成几个字，防止读到上一次截的图
    pyperclip.copy("WAITING")

    # 开启循环，最多偷偷看 20 次（也就是 20 秒）
    for _ in range(20):
        time.sleep(1) # 每隔 1 秒看一眼
        
        # 抓取剪贴板内容
        img = ImageGrab.grabclipboard()
        
        # 如果剪贴板是空的，或者是刚才塞进去的"WAITING"文字，就继续等下一秒
        if img is None or isinstance(img, str):
            continue
...
# 👇 ========================================================= 👇
# 【新增】：F4 手动强制识别（跳过所有等待，直接看一眼剪贴板并识别）
def manual_recognize():
    print("\n[F4 已按下] 正在执行手动强制识别...")
    img = ImageGrab.grabclipboard()
    
    if img is None or isinstance(img, str):
        print("❌ 剪贴板中未检测到有效图片，请确认是否已复制截图。")
        return
...
# 绑定快捷键
keyboard.add_hotkey('f1', recognize_shortcut)
keyboard.add_hotkey('f4', manual_recognize) # 【新增】：绑定 F4 键

print("=====================================================")
print("【双保险运行中】")
print("👉 自动流：按 F1 截图，截完后复制即可自动识别。")
print("👉 强力流：如果自动没反应，按 F4 直接强制识别当前剪贴板！")
print("👉 按 Ctrl+ESC 退出程序。")
print("=====================================================")
